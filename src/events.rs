...
pub trait ProvideLogs<E: EthereumEvent> {
	fn parse(&self, event: &Event, logs: Vec<Log>) -> Result<web3::ethabi::Log, EthereumError> {
		...
		if let Some(log) = logs
			.iter()
			.find(|log| log.topics.iter().any(|topic| topic == &web3_format_hash))
		{
			let parsed_log =
				event.parse_log(RawLog { topics: log.topics.clone(), data: log.data.clone().0 });
...
impl EthereumEvent for LaunchedV2 {
	fn new() -> Self {
		let params = vec![
			EventParam { name: "token".to_string(), kind: ParamType::Address, indexed: false },
			EventParam { name: "escrow".to_string(), kind: ParamType::Address, indexed: false },
			EventParam {
		...
		let event = Event { name: "LaunchedV2".to_string(), inputs: params, anonymous: false };
		Self { event }
	}
}
...
impl EthereumEvent for Transfer {
	fn new() -> Self {
		...
		let event = Event { name: "Transfer".to_string(), inputs: params, anonymous: false };
		Self { event }
	}
}
...
impl EthereumEvent for IntermediateStorage {
	fn new() -> Self {
		let params = vec![
			EventParam { name: "url".to_string(), kind: ParamType::String, indexed: false },
			EventParam { name: "hash".to_string(), kind: ParamType::String, indexed: false },
		];

		let event =
			Event { name: "IntermediateStorage".to_string(), inputs: params, anonymous: false };
		Self { event }
	}
}
...
impl EthereumEvent for TrustedHandlerAdded {
	fn new() -> Self {
		...
		let event =
			Event { name: "TrustedHandlerAdded".to_string(), inputs: params, anonymous: false };
		Self { event }
	}
}
...
impl EthereumEvent for Pending {
	fn new() -> Self {
		...
		let event = Event { name: "Pending".to_string(), inputs: params, anonymous: false };
		Self { event }
	}
}
...
impl EthereumEvent for BulkTransfer {
	fn new() -> Self {
		let params = vec![
			EventParam { name: "txId".to_string(), kind: ParamType::Uint(256), indexed: true },
			EventParam {
				name: "recipients".to_string(),
				kind: ParamType::Array(Box::new(ParamType::Address)),
				indexed: false,
			},
			EventParam { name: "amounts".to_string(), kind: ParamType::Uint(256), indexed: false },
			EventParam { name: "isPartial".to_string(), kind: ParamType::Bool, indexed: false },
		];

		let event = Event { name: "BulkTransfer".to_string(), inputs: params, anonymous: false };
		Self { event }
	}
}
...
impl EthereumEvent for Cancelled {
	fn new() -> Self {
		let event = Event { name: "Cancelled".to_string(), inputs: vec![], anonymous: false };
		Self { event }
	}
}
...
impl EthereumEvent for Completed {
	fn new() -> Self {
		let event = Event { name: "Completed".to_string(), inputs: vec![], anonymous: false };
		Self { event }
	}
}

...
impl KVStore {
	fn new(web3: &web3::Web3<web3::transports::Http>, address: Address) -> ContractResult<KVStore> {
		let contract: Contract<Http> = Contract::from_json(
			web3.eth(),
			address,
			include_bytes!("../abis/KVStore.json"),
		)
...
impl<'a> KVStoreClient<'a> {
	...
	pub async fn new(
		...
		address: Address,
		account: SecretKey,
	) -> ClientResult<KVStoreClient<'a>> {
		let kvstore = KVStore::new(web3, address)?;

		Ok(KVStoreClient { web3, kvstore, account })
	}
	...
	/// # }
	/// ```
	pub async fn set(&self, key: String, value: String) -> Result<(), EthereumError> {
		let result = self
			.kvstore
			.contract
	...
	/// # }
	/// ```
	pub async fn set_url(&self, url: String, url_key: String) -> Result<(), EthereumError> {
	...
	/// # }
	/// ```
	pub async fn get(&self, address: Address, key: String) -> Result<String, EthereumError> {
		if key.is_empty() {
		...
		let value: Address = self
			.kvstore
			.contract
			.query("get", (address, key), None, Options::default(), None)
			.await
	...
	async fn fetch_content(url: &str) -> Result<String, EthereumError> {
		...
		let response = client
			...
			.send()
			.await
			.map_err(|err| EthereumError::RequestError(err.to_string()))?;

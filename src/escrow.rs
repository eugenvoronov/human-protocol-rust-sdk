...
use crate::{
	constants::{CONFIRMATIONS_THRESHOLD, DEFAULT_TX_ID, NETWORKS},
	enums::{ChainId, EscrowStatus, EthereumError},
	events::{EthereumEvent, LaunchedV2, ProvideLogs, Transfer},
	types::{ClientResult, ContractResult},
};
use url::Url;
...
impl EscrowFactory {
	pub fn new(
		...
		address: Address,
	) -> ContractResult<EscrowFactory> {
		let contract: Contract<Http> = Contract::from_json(
			web3.eth(),
			address,
			include_bytes!("../abis/EscrowFactory.json"),
		)
		.map_err(|_| "Failed to load contract ABI")?;

		Ok(EscrowFactory { contract })
	}
}
...
impl Escrow {
	fn new(web3: &web3::Web3<web3::transports::Http>, address: Address) -> ContractResult<Escrow> {
		let contract = Contract::from_json(
			web3.eth(),
			address,
			include_bytes!("../abis/Escrow.json"),
		)
		.map_err(|_| "Failed to load contract ABI")?;
...
impl HMToken {
	pub fn new(
		web3: &web3::Web3<web3::transports::Http>,
		address: Address,
	) -> ContractResult<HMToken> {
		let contract: Contract<Http> = Contract::from_json(
			web3.eth(),
			address,
			include_bytes!("../abis/HMToken.json"),
		)
		.map_err(|_| "Failed to load contract ABI")?;
...
impl<'a> EscrowClient<'a> {
	...
	/// let http = Http::new("https://mainnet.infura.io/v3/YOUR_INFURA_API_KEY")?;
	/// let web3 = web3::Web3::new(http);
	/// let address = Address::from_str("0x1234567890123456789012345678901234567890")?;
	/// let account = SecretKey::from_slice(&[1; 32])?;
	///
	...
	pub async fn new(
		...
		address: Address,
		account: SecretKey,
	) -> ClientResult<EscrowClient<'a>> {
		let escrow_factory = EscrowFactory::new(web3, address)?;

		Ok(EscrowClient { web3, escrow_factory, account })
	}
	...
	pub async fn create_escrow(
		...
		let trusted_handlers: Vec<Address> = trusted_handlers
			...
			.map(|address_str| {
				Address::from_str(address_str)
					.map_err(|_| EthereumError::InvalidEthereumAddressError)
			})
		...
		let result = self
			...
			.signed_call_with_confirmations(
				...
				Options { gas: Some(5_000_000.into()), ..Default::default() },
		...
		match result {
			Ok(receipt) => {
				...
				let log = event.parse(&event.event, receipt.logs)?;
	...
	/// #   let escrow_client = EscrowClient::new(&web3, address, account).await?;
	///
	/// let escrow_address = Address::from_str("0x9876543210987654321098765432109876543210")?;
	...
	pub async fn setup(
		...
		if !self.has_escrow(escrow_address).await? {
	...
	pub async fn create_and_setup_escrow(
		...
		let escrow_address =
			match self.create_escrow(token_address, trusted_handlers, job_requester_id).await {
	...
	/// #     let escrow_client = EscrowClient::new(&web3, address, account).await?;
	///
	/// let escrow_address = Address::from_str("0x17350792fbdf8c584790196222274ef862fef243")?;
	/// let amount = 1000;
	///
	...
	/// # }
	/// ```
	pub async fn fund(&self, escrow_address: Address, amount: u128) -> Result<(), EthereumError> {
		match self.has_escrow(escrow_address).await {
		...
		let status: EscrowStatus = self.get_status(escrow_address).await.map_err(|err| err)?;

		if status != EscrowStatus::Pending {
			return Err(EthereumError::EscrowHasInvalidStatusError);
		}

		let token_address: Address = escrow
			.contract
			.query("token", (), None, Options::default(), None)
			.await
	...
	/// # }
	/// ```
	pub async fn complete(&self, escrow_address: Address) -> Result<(), EthereumError> {
		if !self.has_escrow(escrow_address).await? {
			return Err(EthereumError::EscrowAddressNotProvidedByFactoryError);
		}
	...
	pub async fn bulk_pay_out(
		...
		let balance: u64 = match self.get_balance(escrow_address).await {
			Ok(balance) => balance,
			Err(err) => return Err(err),
		};
		...
		let result = escrow
			...
			.signed_call_with_confirmations(
				"bulkPayOut",
				(recipients, amounts, final_results_url, final_results_hash, DEFAULT_TX_ID),
				Options { gas: Some(5_000_000.into()), ..Default::default() },
				CONFIRMATIONS_THRESHOLD.into(),
				SecretKeyRef::new(&self.account),
			)
	...
	/// # }
	/// ```
	pub async fn cancel(&self, escrow_address: Address) -> Result<EscrowCancel, EthereumError> {
		if !self.has_escrow(escrow_address).await? {
			return Err(EthereumError::EscrowAddressNotProvidedByFactoryError);
		}
		...
		match result {
			Ok(receipt) => {
				...
				let log = event
					.parse(&event.event, vec![log])
					.map_err(|e| EthereumError::LogParsingError(e.to_string()))?;
	...
	/// # }
	/// ```
	pub async fn abort(&self, escrow_address: Address) -> Result<(), EthereumError> {
		if !self.has_escrow(escrow_address).await? {
			return Err(EthereumError::EscrowAddressNotProvidedByFactoryError);
		}
	...
	///     Address::from_str("0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")?,
	/// ];
	///
	...
	/// # }
	/// ```
	pub async fn get_balance(&self, escrow_address: Address) -> Result<u64, EthereumError> {
		if !self.has_escrow(escrow_address).await? {
			return Err(EthereumError::EscrowAddressNotProvidedByFactoryError);
		}
		...
		let balance: u64 = escrow
			.contract
			.query("getBalance", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	pub async fn get_manifest_hash(
		...
		let manifest_hash: String = escrow
			.contract
			.query("manifestHash", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	/// # }
	/// ```
	pub async fn get_manifest_url(&self, escrow_address: Address) -> Result<String, EthereumError> {
		if !self.has_escrow(escrow_address).await? {
			return Err(EthereumError::EscrowAddressNotProvidedByFactoryError);
		}
		...
		let manifest_url: String = escrow
			.contract
			.query("manifestUrl", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	/// # }
	/// ```
	pub async fn get_results_url(&self, escrow_address: Address) -> Result<String, EthereumError> {
		if !self.has_escrow(escrow_address).await? {
			return Err(EthereumError::EscrowAddressNotProvidedByFactoryError);
		}
		...
		let results_url: String = escrow
			.contract
			.query("resultsUrl", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	pub async fn get_intermediate_results_url(
		...
		let intermediate_results_url: String = escrow
			.contract
			.query("intermediateResultsUrl", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	pub async fn get_token_address(
		...
		let token_address: Address = escrow
			.contract
			.query("tokenAddress", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	/// # }
	/// ```
	pub async fn get_status(&self, escrow_address: Address) -> Result<EscrowStatus, EthereumError> {
		if !self.has_escrow(escrow_address).await? {
		...
		let status: u8 = escrow
			.contract
			.query("status", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	pub async fn get_factory_address(
		...
		let factory_address: Address = escrow
			.contract
			.query("factoryAddress", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	pub async fn get_job_launcher_address(
		...
		let job_launcher_address: Address = escrow
			.contract
			.query("jobLauncherAddress", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	pub async fn get_exchange_oracle_address(
		...
		let exchange_oracle_address: Address = escrow
			.contract
			.query("exchangeOracleAddress", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	pub async fn get_recording_oracle_address(
		...
		let recording_oracle_address: Address = escrow
			.contract
			.query("recordingOracleAddress", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	pub async fn get_reputation_oracle_address(
		...
		let reputation_oracle_address: Address = escrow
			.contract
			.query("reputationOracleAddress", (), None, Options::default(), None)
			.await
			.map_err(|web3_error| EthereumError::Web3ContractError(web3_error.to_string()))?;
	...
	pub async fn has_escrow(&self, escrow_address: Address) -> Result<bool, EthereumError> {
		match self
			.escrow_factory
			.contract
...
impl EscrowUtils {
	...
	pub async fn get_escrows(
		...
		filter: escrows_query::EscrowFilter,
	) -> Result<Vec<escrows_query::EscrowData>, Box<dyn Error>> {
		let mut escrow_addresses = Vec::new();
		for chain_id in networks {
			let network = NETWORKS
				.get(&chain_id)
				.ok_or_else(|| EthereumError::NetworkNotFoundForChainIdError(chain_id as u32))?;
	...
	pub async fn get_escrow(
		chain_id: ChainId,
		filter: escrow_query::EscrowFilter,
	) -> Result<escrow_query::EscrowData, EthereumError> {
		let network = NETWORKS
			.get(&chain_id)
			.ok_or_else(|| EthereumError::NetworkNotFoundForChainIdError(chain_id as u32))?;

		match escrow_query::run_query(network.subgraph_url, &filter).await.data {
			Some(escrow_query::GetEscrowByAddressQuery { escrow: Some(escrow) }) => Ok(escrow),
			_ => return Err(EthereumError::EscrowNotFoundForEscrowAddressError(filter.id.into())),
		}
	}
}
...
mod tests {
	...
	#[tokio::test]
	async fn test_escrow_factory_creation() -> Result<(), EthereumError> {
		let http = Http::new("https://mainnet.infura.io/v3/YOUR_INFURA_API_KEY")?;
		let web3 = web3::Web3::new(http);
		let address = Address::from_str("0x1234567890123456789012345678901234567890")
			.map_err(|_| EthereumError::InvalidKeyError)?;
	...
	#[tokio::test]
	async fn test_escrow_creation() -> Result<(), EthereumError> {
		let http = Http::new("https://mainnet.infura.io/v3/YOUR_INFURA_API_KEY")?;
		let web3 = web3::Web3::new(http);
		let address = Address::from_str("0x1234567890123456789012345678901234567890")
			.map_err(|_| EthereumError::InvalidKeyError)?;
	...
	#[tokio::test]
	async fn test_hmt_token_creation() -> Result<(), EthereumError> {
		let http = Http::new("https://mainnet.infura.io/v3/YOUR_INFURA_API_KEY")?;
		let web3 = web3::Web3::new(http);
		let address = Address::from_str("0x1234567890123456789012345678901234567890")
			.map_err(|_| EthereumError::InvalidKeyError)?;
	...
	#[tokio::test]
	async fn test_escrow_config_default() -> Result<(), EthereumError> {
	...
	#[tokio::test]
	async fn test_escrow_client_creation() -> Result<(), EthereumError> {
		let http = Http::new("https://mainnet.infura.io/v3/YOUR_INFURA_API_KEY")?;
		let web3 = web3::Web3::new(http);
		let address = Address::from_str("0x1234567890123456789012345678901234567890")
			.map_err(|_| EthereumError::InvalidKeyError)?;
		let account =
			SecretKey::from_str("75c64a0d8a1fac998cfc1165fe0d8975be1e8ebe8f5bb52cfde3339a6e94e7a5")
				.map_err(|_| EthereumError::InvalidKeyError)?;
	...
	#[tokio::test]
	async fn test_create_escrow_invalid_address() -> Result<(), EthereumError> {
		let http = Http::new("https://mainnet.infura.io/v3/YOUR_INFURA_API_KEY")?;
		let web3 = web3::Web3::new(http);
		let account =
			SecretKey::from_str("75c64a0d8a1fac998cfc1165fe0d8975be1e8ebe8f5bb52cfde3339a6e94e7a5")
		...
		let escrow_client = EscrowClient { web3: &web3, escrow_factory, account };
		...
		assert_eq!(result.unwrap_err(), EthereumError::InvalidEthereumAddressError);
		Ok(())
	}
}

...
use crate::{
	constants::NETWORKS,
	enums::{ChainId, Error, EthereumError},
	escrow::{EscrowFactory, HMToken},
	...
	fn from_token(token: Token) -> Result<Self, web3::contract::Error> {
		let data = if let Token::Tuple(value) = token {
			...
			return Err(web3::contract::Error::InvalidOutputType(
				Error::InvalidTokenConversionError.to_string(),
			));
		};

		if data.len() != 5 {
			return Err(web3::contract::Error::InvalidOutputType(
				Error::InvalidTupleLengthError.to_string(),
			));
		}
		...
		Ok(Allocation { escrow_address, staker, tokens, created_at, closed_at })
	}
...
impl Staking {
	fn new(web3: &web3::Web3<web3::transports::Http>, address: Address) -> ContractResult<Staking> {
		let contract: Contract<Http> = Contract::from_json(
			web3.eth(),
			address,
			include_bytes!("../abis/Staking.json"),
		)
...
impl<'a> StakingClient<'a> {
	...
	/// Returns a `Result`:
	///
	/// - `Ok(StakingClient<'a>)` on successful creation of the `StakingClient` instance.
	/// - `Err(ClientError)` on failure, where `ClientError` can include errors from the contract loading
	///   process, Web3 connection errors, or other internal issues.
	///
	/// The `?` operator is used for error propagation, meaning if an error occurs during the construction
	/// of any of the contract instances, it will be returned early to the caller, preventing further execution.
	///
	/// # Example
	///
	/// ```rust
	...
	/// let staking_address = Address::from_str("0x1234567890123456789012345678901234567890")?;
	/// let token_address = Address::from_str("0x1234567890123456789012345678901234567891")?;
	/// let escrow_factory_address = Address::from_str("0x1234567890123456789012345678901234567892")?;
	///
	/// let staking_client = StakingClient::new(&web3, staking_address, token_address, escrow_factory_address, account).await?;
	///
	/// #     Ok(())
	/// # }
	/// ```
	pub async fn new(
		...
		Ok(StakingClient { web3, staking, token, escrow_factory, account })
	}
	...
	/// # }
	/// ```
	pub async fn approve_stake(&self, amount: u64) -> Result<(), EthereumError> {
		if amount == 0 {
		...
		let result: Result<_, _> = self
			...
			.signed_call_with_confirmations(
				"approve",
				(self.staking.contract.address(), amount),
	...
	/// # }
	/// ```
	pub async fn stake(&self, amount: u64) -> Result<(), EthereumError> {
		if amount == 0 {
			return Err(EthereumError::NonPositiveValueError);
		}
	...
	/// # }
	/// ```
	pub async fn unstake(&self, amount: u64) -> Result<(), EthereumError> {
		if amount == 0 {
			return Err(EthereumError::NonPositiveValueError);
		}
	...
	/// # }
	/// ```
	pub async fn withdraw(&self) -> Result<(), EthereumError> {
		let result: Result<_, _> = self
			.staking
			.contract
			.signed_call_with_confirmations(
	...
	/// # let staking_client = StakingClient::new(&web3, staking_address, token_address, escrow_factory_address, account).await?;
	///
	/// let slasher_address = Address::from_str("0x1234567890123456789012345678901234567890")?;
	/// let staker_address = Address::from_str("0x1234567890123456789012345678901234567890")?;
	/// let escrow_address = Address::from_str("0x1234567890123456789012345678901234567890")?;
	/// let amount: u64 = 100;
	///
	...
	/// # }
	/// ```
	pub async fn close_allocation(&self, escrow_address: Address) -> Result<(), EthereumError> {
		if !self
			.escrow_factory
			.contract
			.query("hasEscrow", escrow_address.clone(), None, Options::default(), None)
	...
	/// # }
	/// ```
	pub async fn distribute_rewards(&self, escrow_address: Address) -> Result<(), EthereumError> {
		if !self
			.escrow_factory
			.contract
			.query("hasEscrow", escrow_address.clone(), None, Options::default(), None)
	...
	pub async fn get_allocation(
		...
		let result: Allocation = self
			.staking
			.contract
			.query("getAllocation", escrow_address, None, Options::default(), None)
			.await
...
impl StakingUtils {
	...
	pub async fn get_leaders(
		...
		match staking_leaders_query::run_query(network.subgraph_url, &filter).await.data {
			Some(staking_leaders_query::GetLeadersQuery { leaders: Some(leaders) }) => Ok(leaders),
			_ => return Err(EthereumError::LeadersNotFoundError),
		}
	}
	...
	pub async fn get_leader(
		...
		match staking_leader_query::run_query(network.subgraph_url, &filter).await.data {
			Some(staking_leader_query::GetLeaderQuery { leader: Some(leader) }) => Ok(leader),
			_ => return Err(EthereumError::LeaderNotFoundError(filter.id.into())),
		}
	}
	...
	pub async fn get_rewards(
		...
		match staking_rewards_query::run_query(network.subgraph_url, &filter).await.data {
			Some(staking_rewards_query::GetRewardsQuery {
				reward_added_events: Some(reward_added_events),
			}) => Ok(reward_added_events),
			_ => {
				return Err(EthereumError::RewardsNotFoundForSlasherAddressError(
					filter.slasher.into(),
				))
			},
		}

...
impl Default for LeaderFilter {
	fn default() -> Self {
		Self { role: Some(Role::to_string_vec()) }
	}
}
...
pub async fn run_query(
	url: &str,
	filter: &LeaderFilter,
) -> cynic::GraphQlResponse<GetLeadersQuery> {
...
mod test {
	...
	async fn test_filter_builder() {
		let filter = LeaderFilter::default().role(Some(vec!["some_role".to_string()])).build();

...
pub async fn run_query(
	url: &str,
	filter: &RewardFilter,
) -> cynic::GraphQlResponse<GetRewardsQuery> {
...
mod test {
	...
	async fn snapshot_test_query() {
		let filter = RewardFilter { slasher: "some_address".to_string() };
		let query = build_query(&filter);
	...
	async fn test_running_query() {
		...
		let filter =
			RewardFilter { slasher: "0x1234567890123456789012345678901234567890".to_string() };

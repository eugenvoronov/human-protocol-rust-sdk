...
pub async fn run_query(
	url: &str,
	filter: &EscrowFilter,
) -> cynic::GraphQlResponse<GetEscrowByAddressQuery> {
...
mod test {
	...
	async fn snapshot_test_query() {
		let filter = EscrowFilter { id: "some_escrow_address".into() };
		let query = build_query(&filter);
	...
	async fn test_running_query() {
		let filter = EscrowFilter { id: "0x01fc22166dbf5743472ea56e2a1bc8372377e402".into() };
		let result =
			run_query("https://api.thegraph.com/subgraphs/name/humanprotocol/mumbai-v2", &filter)
				.await;

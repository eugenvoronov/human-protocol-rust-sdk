...
mod test {
	...
	async fn snapshot_test_query() {
		let filter = LeaderFilter { id: "some_address".to_string() };
		let query = build_query(&filter);
	...
	async fn test_running_query() {
		...
		let filter = LeaderFilter { id: "0x9d1f5d4c4aa1764bf63583c3392f626e2ffbe5f7".to_string() };
	...
	async fn test_filter_builder() {
		let filter = LeaderFilter { id: "some_address".into() };

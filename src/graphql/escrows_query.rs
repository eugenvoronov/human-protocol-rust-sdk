...
pub async fn run_query(
	url: &str,
	filter: &EscrowFilter,
) -> cynic::GraphQlResponse<GetEscrowsQuery> {
	use cynic::http::ReqwestExt;

	let query = build_query(filter);

	reqwest::Client::new().post(url).run_graphql(query).await.unwrap()
}
...
#[cfg(test)]
mod test {
	use crate::{constants::NETWORKS, enums::ChainId};
	...
	async fn test_filter_builder() {
		...
		assert_eq!(filter.exchange_oracle, Some("some_exchange_oracle".to_string()));
		assert_eq!(filter.recording_oracle, Some("some_recording_oracle".to_string()));
		assert_eq!(filter.reputation_oracle, Some("some_reputation_oracle".to_string()));
		assert_eq!(filter.job_requester_id, Some("some_job_requester_id".to_string()));
		assert_eq!(filter.status, Some(vec!["status1".to_string(), "status2".to_string()]));
